...
public class VuforiaNavigation {
        ...
        for (VuforiaTrackable trackable : trackables) {
            ...
            if (listener.isVisible()) {
                ...
                if (robotLocation != null) {
                    updateLocationFromMatrix(robotLocation);
                }
                break;
            }
        }
    ...
    private void updateLocationFromMatrix(OpenGLMatrix locationMatrix) {
        this.position = referenceFrame.getRelativePosition(locationMatrix.getTranslation().multiplied(1 / Constants.MM_PER_INCH));
        Orientation rotation = Orientation.getOrientation(locationMatrix, EXTRINSIC, XYZ, DEGREES);
